...
DEFAULT_JOBS_PATH = os.path.join(os.path.dirname(__file__), "jobs")


def cli() -> None:
	signal.signal(signal.SIGINT, lambda signal_number, frame: graceful_exit(0))
	state_manager.init_item('jobs_path', DEFAULT_JOBS_PATH)
...
def route(args : Args) -> None:
	...
	if state_manager.get_item('command') == 'run':
		...
		if not job_manager.init_jobs("jobs"):  # ⬅️ תקייה בשם jobs
			hard_exit(1)
